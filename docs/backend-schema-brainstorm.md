# Evidence-Based Vocabulary App Backend Schema Plan
...
Possible `learning_stage` values:
...
Do not store a user's mastery level directly on the global `words` table.
...
Because mastery belongs to the user, not to the word or the global sense.
...
Add these only after the lean MVP is working:
